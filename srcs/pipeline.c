...
/*   Updated: 2022/01/12 08:52:13 by jodufour         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	t_uint const i)
{
	if (!i)
	{
		if (pipe(ctx->fds.tube) == -1)
			return (EXIT_FAILURE);
		if (__first(ctx, *cmd_arr) || dup2(ctx->fds.tube[0], ctx->fds.save) < 0)
			return (ft_fddel(&ctx->fds.tube[0]) | ft_fddel(&ctx->fds.tube[1])
				| EXIT_FAILURE);
		if (ft_fddel(&ctx->fds.tube[0]) | ft_fddel(&ctx->fds.tube[1])
			|| pipeline(ctx, cmd_arr + 1, cmd_cnt, i + 1))
			return (EXIT_FAILURE);
	}
	else if (i + 1 < cmd_cnt)
	{
		if (pipe(ctx->fds.tube) == -1)
			return (EXIT_FAILURE);
		if (__inter(ctx, *cmd_arr) || dup2(ctx->fds.tube[0], ctx->fds.save) < 0)
			return (ft_fddel(&ctx->fds.tube[0]) | ft_fddel(&ctx->fds.tube[1])
				| EXIT_FAILURE);
		if (ft_fddel(&ctx->fds.tube[0]) | ft_fddel(&ctx->fds.tube[1])
			|| pipeline(ctx, cmd_arr + 1, cmd_cnt, i + 1))
